...
function getMusicEmbed(url: string) {
    const youtubeId = getYoutubeId(url);
    if (youtubeId) return `https://www.youtube.com/embed/${youtubeId}?autoplay=1&modestbranding=1&showinfo=0&rel=0`;
...
function SearchContent() {
    ...
    useEffect(() => {
        const fetchAudioStream = async () => {
            ...
            const embedType = getMusicEmbed(activeMusicTrack.url);
            if (embedType === 'EXTERNAL_SOURCE') {
                setIsAudioLoading(true);
                setExtractedAudioUrl(null);
                try {
                    const res = await fetch(`/api/audio-extract?url=${encodeURIComponent(activeMusicTrack.url)}`);
                    if (res.ok) {
                        const data = await res.json();
                        if (data.url) setExtractedAudioUrl(data.url);
                    }
                } catch (e) {
        ...
        <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-2 mb-6 animate-fade-in">
                                {allResults.map((result: any, index) => {
                                    ...
                                    return (
                                        <div
                                            key={index}
                                            onClick={() => setActiveMusicTrack(result)}
                                ...
                                {allResults.map((result: any, index) => {
                                    ...
                                    <a key={index} href={result.url} target="_blank" rel="noreferrer"
                                        ...
                                        <div className="flex items-center gap-3 mb-3">
                                            ...
                                            {result.publishedDate && <span className="text-[10px] font-bold text-purple-500 ml-auto whitespace-nowrap">{formatNewsDate(result.publishedDate)}</span>}
                                        </div>
            ...
            {activeMusicTrack && (
                <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
                    ...
                    <div className="max-w-2xl w-full bg-[var(--card)] border border-[var(--card-border)] rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-scale-in">
                        <div className="relative p-8 flex flex-col items-center justify-center bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-pink-500/10 min-h-[350px]">
                            {/* Decorative Background Elements */}
                            <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500" />
                            
                            {/* Vinyl Record Animation */}
                            <div className="relative group">
                                <div className="w-48 h-48 rounded-full bg-black shadow-2xl relative flex items-center justify-center overflow-hidden animate-spin-slow">
                                    {/* Vinyl Grooves */}
                                    ...
                                    <div className="absolute inset-8 border-[1px] border-white/10 rounded-full" />
                                    <div className="absolute inset-12 border-[1px] border-white/10 rounded-full" />
                                    
                                    {/* Cover Image in Center */}
                                    <div className="w-16 h-16 rounded-full bg-[var(--card-border)] overflow-hidden border-2 border-black z-10">
                                        <img 
                                            src={activeMusicTrack.img_src || activeMusicTrack.thumbnail_src || (getYoutubeId(activeMusicTrack.url) ? `https://img.youtube.com/vi/${getYoutubeId(activeMusicTrack.url)}/hqdefault.jpg` : '/favicon.ico')} 
                                            alt="" 
                                ...
                                {/* Needle */}
                                <div className="absolute -top-4 -right-8 w-1 h-24 bg-gray-400 origin-top rotate-[25deg] rounded-full shadow-lg transition-transform group-hover:rotate-[28deg]">
                                    <div className="absolute bottom-0 -left-1 w-3 h-3 bg-gray-300 rounded-full" />
                                </div>
                            </div>

                            {/* Info */}
                            <div className="mt-10 text-center">
                                <h3 className="font-extrabold text-xl text-[var(--foreground)] line-clamp-1 px-4">
                                    {activeMusicTrack.title}
                                </h3>
                                <div className="flex items-center justify-center gap-2 mt-2">
                                    <span className="px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-500 text-[10px] font-bold uppercase tracking-widest">
                                        Music Result
                                    </span>
                                    <span className="text-sm text-[var(--muted)]">
                                        {getDomain(activeMusicTrack.url)}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="w-full">
                            {(() => {
                                const embedUrl = getMusicEmbed(activeMusicTrack.url);
                                
                                if (embedUrl === 'AUDIO_FILE') {
                                    return (
                                        <div className="p-8 flex flex-col items-center">
                                            ...
                                            />
                                            <p className="text-[10px] text-[var(--muted)] mt-2 font-mono">Direct Audio Stream</p>
                                        </div>
                                    );
                                }
                                
                                if (embedUrl === 'EXTERNAL_SOURCE') {
                                    return (
                                        <div className="p-12 text-center flex flex-col items-center gap-4 bg-black/5 dark:bg-white/5">
                                            {isAudioLoading ? (
                                                <div className="flex flex-col items-center gap-4">
                                                    <div className="w-12 h-12 border-4 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin" />
                                                    <p className="text-xs font-bold text-indigo-500 animate-pulse uppercase tracking-widest">Smart Extraction in Progress...</p>
                                                </div>
                                            ) : extractedAudioUrl ? (
                                                <div className="w-full flex flex-col items-center gap-4">
                                                    ...
                                                    />
                                                    <p className="text-[10px] text-green-500 font-bold uppercase">Stream Successfully Extracted</p>
                                                </div>
                                            ) : (
                                                <>
                                                    <div className="w-16 h-16 rounded-2xl bg-[var(--card)] border border-[var(--card-border)] flex items-center justify-center shadow-sm">
                                                        <ExternalLink className="w-8 h-8 text-indigo-500" />
                                                    </div>
                                                    <div>
                                                        <p className="font-bold text-[var(--foreground)]">Direct Playback Unavailable</p>
                                                        <p className="text-sm text-[var(--muted)] mt-1">This source protects its stream. Please listen on the official site.</p>
                                                    </div>
                                                    <a 
                                                        href={activeMusicTrack.url} 
                                                        target="_blank" 
                                                        rel="noreferrer"
                                                        className="px-6 py-2.5 rounded-full bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 transition-colors"
                                                    >
                                                        Open in New Tab
                                                    </a>
                                                </>
                                ...
                                if (embedUrl) {
                                    return (
                                        <div className={`${embedUrl.includes('spotify') ? 'h-[80px]' : 'h-0'} w-full overflow-hidden transition-all duration-500`}>
                                            <iframe 
                                                src={embedUrl}
                                                className="w-full h-full border-0"
                        ...
                        {/* Hidden Iframe for Background Playback (only for YouTube/SoundCloud if not already visible) */}
                        {(() => {
                            const embedUrl = getMusicEmbed(activeMusicTrack.url);
                            if (embedUrl && embedUrl !== 'AUDIO_FILE' && embedUrl !== 'EXTERNAL_SOURCE' && !embedUrl.includes('spotify')) {
                                return (
                                    <div className="h-0 w-0 overflow-hidden opacity-0 pointer-events-none">
                                        <iframe 
                                            src={embedUrl} 
                                            allow="autoplay; encrypted-media"
                                        />
                        ...
                        {/* Controls & Footer */}
                        <div className="p-8 bg-[var(--card)] border-t border-[var(--card-border)]">
                            <div className="flex flex-col gap-4">
                                <p className="text-sm text-[var(--muted)] text-center line-clamp-2 italic">
                                    &quot;{activeMusicTrack.snippet}&quot;
                                </p>
                                <div className="flex items-center justify-center gap-4 mt-2">
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="flex-1 py-3 px-6 rounded-2xl bg-indigo-600 text-white text-center text-sm font-bold shadow-xl shadow-indigo-500/25 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2"
                                    >
                                    ...
                                    <button
                                        onClick={() => setActiveMusicTrack(null)}
                                        className="py-3 px-6 rounded-2xl bg-[var(--background)] border border-[var(--card-border)] text-[var(--foreground)] text-sm font-bold hover:bg-[var(--card-border)]/30 transition-colors"
                                    >
                                        Close Player
                                    </button>
                                </div>
